...
describe('/tags', () => {
  ...
  describe('GET /tags', () => {
    ...
    it('should return a nested tags', async () => {
      ...
      expect(body).toEqual([
        expect.objectContaining({ name: 'TagA', path: 'TagA' }),
        expect.objectContaining({ name: 'TagB', path: 'TagA/TagB' }),
        expect.objectContaining({ name: 'TagC', path: 'TagA/TagB/TagC' }),
        expect.objectContaining({ name: 'TagD', path: 'TagD' }),
      ]);
      expect(status).toEqual(200);
    });
  });
  ...
  describe('PUT /tags', () => {
    ...
    it('should upsert tags', async () => {
      ...
      expect(body).toEqual([expect.objectContaining({ name: 'TagD', path: 'TagA/TagB/TagC/TagD' })]);
    });
  });
